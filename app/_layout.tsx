...
function OnboardingWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const handleFinish = useCallback(async () => {
    if (selectedName) {
      await AsyncStorage.setItem(COLLECTOR_STORAGE_KEY, selectedName);
    }
    if (selectedRig) {
      await AsyncStorage.setItem(RIG_STORAGE_KEY, selectedRig);
    }
  ...
  return (
    <View style={[obStyles.container, { backgroundColor: bgColor }]}>
      ...
      <Animated.View style={[obStyles.stepContent, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        {step === 0 && (
          <View style={obStyles.stepInner}>
            <Text style={[obStyles.title, { color: colors.accent, fontFamily: FONT_MONO }]}>
              {t.onboarding.welcome}
            </Text>
            <Text style={[obStyles.subtitle, { color: colors.textSecondary }]}>
        ...
        {step === 1 && (
          <View style={obStyles.stepInner}>
            ...
            <Text style={[obStyles.title, { color: colors.accent, fontFamily: FONT_MONO }]}>
              {t.onboarding.selectName}
            </Text>
            <Text style={[obStyles.subtitle, { color: colors.textSecondary }]}>
              {t.onboarding.selectNameSub}
            </Text>
              ...
              <View style={[obStyles.listCard, { backgroundColor: cardBg, borderColor }]}>
                <FlatList
                  ...
                  renderItem={({ item }) => {
                    ...
                    return (
                      <TouchableOpacity
                        style={[obStyles.listItem, {
                          backgroundColor: isSelected ? colors.accentSoft : "transparent",
                        }]}
                        onPress={() => { setSelectedName(item.name); setSelectedRig(""); }}
                        activeOpacity={0.7}
                      >
                        <View style={obStyles.listItemLeft}>
                          <Text style={[obStyles.listItemName, {
                          ...
                          {item.rigs.length > 0 && (
                            <Text style={[obStyles.listItemSub, { color: colors.textMuted }]}>
                              {item.rigs.join(", ")}
                            </Text>
                          )}
                        </View>
        ...
        {step === 2 && (
          <View style={obStyles.stepInner}>
            ...
            <Text style={[obStyles.title, { color: colors.accent, fontFamily: FONT_MONO }]}>
              {t.onboarding.selectRig}
            </Text>
            <Text style={[obStyles.subtitle, { color: colors.textSecondary }]}>
              {t.onboarding.selectRigSub}
            </Text>
            ...
            {rigs.length > 0 ? (
                  ...
                  renderItem={({ item }) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Text style={[obStyles.rigItemText, {
                          color: isSelected ? colors.accent : colors.textPrimary,
              ...
              <View style={[obStyles.noRigsCard, { backgroundColor: inputBg, borderColor }]}>
                <Text style={[obStyles.noRigsText, { color: colors.textMuted }]}>
                  {t.onboarding.noRigs}
                </Text>
              </View>
            )}
      ...
      <View style={obStyles.navRow}>
        {step > 0 ? (
          <TouchableOpacity
            style={[obStyles.backBtn, { borderColor }]}
        ...
        {step < TOTAL_STEPS - 1 ? (
          <TouchableOpacity
            style={[obStyles.nextBtn, {
              ...
              opacity: step === 1 && !selectedName ? 0.4 : 1,
            }]}
            onPress={() => animateStep(step + 1)}
          ...
          <TouchableOpacity
            style={[obStyles.nextBtn, { backgroundColor: colors.accent }]}
...
const obStyles = StyleSheet.create({
  ...
  rigItem: {
    ...
    paddingHorizontal: 16, paddingVertical: 14, marginHorizontal: 10, marginVertical: 4,
...
function BootSequence({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    Animated.spring(enterScale, { toValue: 1, speed: 10, bounciness: 8, useNativeDriver: true, delay: 200 }).start();
  ...
  return (
    <Animated.View style={[bootStyles.container, { backgroundColor: bgColor, opacity: fadeOut }]}>
      <Animated.View style={[bootStyles.logoWrap, { opacity: logoOpacity }]}>
        <Text style={[bootStyles.logoText, { color: accentColor, fontFamily: FONT_MONO }]}>TASKFLOW</Text>
      ...
      <View style={bootStyles.terminalArea}>
        {lines.map((line, idx) => (
          <Text key={`boot_${idx}`} style={[bootStyles.termLine, {
            color: line.startsWith("\u25B8") ? dimColor : accentColor, fontFamily: FONT_MONO,
          }]}>
            {line}
          </Text>
        ))}
      ...
      {phase === "ready" && (
        <Animated.View style={[bootStyles.enterWrap, { transform: [{ scale: enterScale }] }]}>
          ...
          >
            <Text style={[bootStyles.enterText, { color: '#FFFFFF', fontFamily: FONT_MONO }]}>ENTER SYSTEM</Text>
          </TouchableOpacity>
        </Animated.View>
      )}
    </Animated.View>
...
const bootStyles = StyleSheet.create({
  ...
  terminalArea: { width: SCREEN_WIDTH * 0.85, maxWidth: 380, minHeight: 140, paddingBottom: 8 },
  termLine: { fontSize: 11, lineHeight: 20, letterSpacing: 0.3 },
  cursor: { fontSize: 12, lineHeight: 20 },
  progressWrap: { width: SCREEN_WIDTH * 0.6, maxWidth: 280, marginTop: 32, alignItems: "center", gap: 8 },
